  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/initial-data' && request.method === 'GET') {
        ...
        if (!auth.ok) return withCors(json({ error: 'Unauthorized' }, 401), cors);
        return withCors(await handleInitialData(env), cors);
      }

      if (url.pathname === '/api/row-values' && request.method === 'GET') {
        const auth = await verifyAuth(request, env);
        if (!auth.ok) return withCors(json({ error: 'Unauthorized' }, 401), cors);
        return withCors(await handleRowValues(url, env), cors);
      }

      if (url.pathname === '/api/update' && request.method === 'POST') {
        const auth = await verifyAuth(request, env);
        if (!auth.ok) return withCors(json({ error: 'Unauthorized' }, 401), cors);
        return withCors(await handleUpdate(request, env, auth.user), cors);
      }

      return withCors(json({ error: 'Not found' }, 404), cors);
    } catch (err) {
      return withCors(json({ error: err.message || 'Internal error' }, 500), cors);
    }
  }
};
...
async function handleInitialData(env) {
  const data = await getSheetValues(env, env.DATA_SHEET_NAME);
  if (!data.length) {
  ...
  const legend = {};
  const legendData = await getSheetValues(env, env.LEGEND_SHEET_NAME).catch(() => []);
...
async function handleUpdate(request, env, user) {
  ...
  for (let i = 1; i < data.length; i += 1) {
    ...
    const matchSegment = isAllSegments
      ? true
      : String(row[idxSegment] || '').trim() === String(payload.segment || '').trim();
  ...
  if (changeLogs.length > 0) {
    await updateValues(env, `${env.DATA_SHEET_NAME}!A1`, data, 'USER_ENTERED');

    await ensureSheetExists(env, env.LOG_SHEET_NAME);
    const existingLog = await getSheetValues(env, env.LOG_SHEET_NAME).catch(() => []);
    if (!existingLog.length) {
      await updateValues(env, `${env.LOG_SHEET_NAME}!A1`, [[
        'Timestamp',
        'User',
        'Tactic',
        'Vertical',
    ...
    await appendValues(env, `${env.LOG_SHEET_NAME}!A:H`, changeLogs, 'USER_ENTERED');
  }
...
function corsHeaders(env, origin) {
  ...
  const useOrigin = allowed === '*' ? '*' : allowed;
...
async function verifyAuth(request, env) {
  ...
  try {
    ...
    const expected = await hmacSha256Base64Url(env.SESSION_SECRET, data);
...
async function getGoogleAccessToken(env) {
  ...
  if (!resp.ok) throw new Error(jsonResp.error_description || jsonResp.error || 'Failed to get Google access token');
...
async function updateValues(env, range, values, valueInputOption) {
  ...
  await sheetsFetch(env, `/values/${encodedRange}?valueInputOption=${encodeURIComponent(valueInputOption)}`, 'PUT', {
    values
  });
}
...
async function appendValues(env, range, values, valueInputOption) {
  ...
  await sheetsFetch(
    env,
    `/values/${encodedRange}:append?valueInputOption=${encodeURIComponent(valueInputOption)}&insertDataOption=INSERT_ROWS`,
    'POST',
    { values }
  );
}
