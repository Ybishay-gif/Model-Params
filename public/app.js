const SLIDERS = {
  'slider-roe': {
    keys: ['roe_poor', 'roe_minimal', 'roe_0', 'roe_good', 'roe_excellent', 'roe_amazing'],
    min: -50,
    max: 50,
    step: 0.1
  },
  'slider-perf': {
    keys: ['per_poor', 'per_minimal', 'per_good', 'per_excellent', 'per_amazing'],
    min: 0,
    max: 300,
    step: 1
  },
  'slider-wr': {
    keys: ['Poor_WR', 'Low_WR', 'OK_WR', 'High_WR', 'VHigh_WR'],
    min: 0,
    max: 100,
    step: 0.1
  },
  'slider-quote': {
    keys: ['QuoteRate_poor', 'QuoteRate_minimal', 'QuoteRate_good', 'QuoteRate_excellent', 'QuoteRate_amazing'],
    min: 0,
    max: 100,
    step: 0.1
  }
...
const GROUPS = {
  'sec-dates': [
    'early_funnel_start_days',
    'early_funnel_end_days',
    'early_cmp_funnel_start_days',
    'early_cmp_funnel_end_days',
    'perf_start_days',
    'perf_end_days'
  ],
...
['tactic', 'vertical', 'segment'].forEach((id) => {
  document.getElementById(id).addEventListener('change', fetchCurrentValues);
});
    ...
    document.getElementById(id).innerHTML = GROUPS[id]
      .map(
        ...
        <div class="col-md-4 mb-3">
          <label class="small fw-bold mb-1">${legend[key]?.displayName || key}${
          id === 'sec-cost' ? ' ($)' : ''
        }</label>
  ...
  try {
    ...
    if (vals) {
      ...
      Object.keys(vals).forEach((k) => {
        const el =
          document.getElementById(`in-${k}`) ||
          Array.from(document.querySelectorAll('.val-input')).find(
            (input) => input.dataset.key.toLowerCase() === k.toLowerCase()
          );
