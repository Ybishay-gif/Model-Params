...
function parseStrategyRules(raw) {
    try {
...
function splitCombinedFilter(value) {
...
export async function getStrategyAnalysis(filters) {
    ...
    const mergedByChannelStateSegment = new Map();
    ...
    for (const groupRows of mergedByChannelStateSegment.values()) {
        const ranked = [...groupRows].sort((a, b) => {
            ...
            const aCpbUplift = Number.isFinite(Number(a.expected_cpb)) && Number.isFinite(Number(a.ss_cpb)) && Number(a.ss_cpb) !== 0
                ? (Number(a.expected_cpb) - Number(a.ss_cpb)) / Number(a.ss_cpb)
                : null;
            const bCpbUplift = Number.isFinite(Number(b.expected_cpb)) && Number.isFinite(Number(b.ss_cpb)) && Number(b.ss_cpb) !== 0
            ...
            const aPriority = aTp !== 0 && aCpbUplift !== null && aCpbUplift <= 0.10 && aAdditionalClicks > 0 ? 0 : aTp === 0 ? 1 : 2;
            const bPriority = bTp !== 0 && bCpbUplift !== null && bCpbUplift <= 0.10 && bAdditionalClicks > 0 ? 0 : bTp === 0 ? 1 : 2;
    ...
    return rules.map((rule) => {
        ...
        const matchingRecommended = recommendedRows.filter((row) => stateSet.has(String(row.state || "").toUpperCase()) && segmentSet.has(String(row.segment || "").toUpperCase()));
        ...
        const additionalBinds = matchingRecommended.reduce((sum, row) => sum + (Number(row.additional_expected_binds) || 0), 0);
        ...
        const cpbWeighted = matchingRecommended.reduce((acc, row) => {
